...
contract MortalGame is ERC20 {
    ...
    function directBuyNFTs(uint _tokenAmount, uint _NFTID) external {
        require(
            ...
            "Not Enough tokens in the account"
        );
        bool res = transfer(address(this), _tokenAmount);
        require(res, "Not successful");
        if (
            (_NFTID < gameAsset.totalAssetsMinted()) &&
            gameAsset.balanceOf(address(this)) > 0 &&
            gameAsset.ownerOf(_NFTID) == address(this)
        ) {
    ...
    function rewardTokens(address _receiver, uint _amount) external {
        if (msg.sender == owner) {
            require(
                ...
                "Not enought Tokens in the liquidity pool"
            );
    ...
    function auctionBuyNFT(uint _tokenAmount) external {
        require(
            balanceOf(msg.sender) >= _tokenAmount,
            "Not enough token with the bidder"
        );
    ...
    function finishAuction() external onlyOwner {
        if (auction[auctionCounter].seller != address(0)) {
            ...
            _transfer(
                address(this),
                auction[auctionCounter].seller,
                lastHighest - profit
            );
            gameAsset.transferAsset(
                (address(this)),
                lastHighestOwner,
                auction[auctionCounter].NFTID
            );
            auctionCounter++;
            lastHighest = 0;
    ...
    function playWithSystem(uint _choice) internal returns (bool) {
        ...
        if (res == true && inLeaderBoard[msg.sender] != false) {
            ...
            leaderBoard[leaderBoardIndex[msg.sender]].score =
                leaderBoard[leaderBoardIndex[msg.sender]].score +
                1;
            return true;
        } else {
    ...
    function guessTheNumberAgainstSystem(
        uint _inputNumber
    ) external returns (string memory) {
